...
- **快速模板**：Burp Suite (8080) / Clash HTTP (7890) / Clash SOCKS5 (7891) / v2rayN (10808) / SSH 隧道 (1080) / 空白
- 批量保存 + 行内校验（名称唯一、host 非空、端口 1~65535），非法行红框提示

### 自动切换（SwitchyOmega 风格）🆕

- 规则列表：**域名表达式 → 目标**（直连 / 任意代理配置），按顺序匹配
- 表达式语义与 SwitchyOmega 一致：`*.example.com` 匹配主域与全部子域名、`*` 匹配所有
